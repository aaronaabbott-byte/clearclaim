...
// No-AI fallback: conservative keyword match against the closed core list.
// Anything not clearly on the list is non-core per 35-102(4).
...
export function validateVerdict(v) {
  const ids = coreIds();
  const out = {
    classification: ["core", "non-core", "ambiguous"].includes(v.classification) ? v.classification : "ambiguous",
