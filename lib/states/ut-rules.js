...
// Utah caps are a percentage of the STUDENT'S award (which varies by age/
// setting), except transportation which is a flat $750/year.
export function utCategoryCapKey(category) {
  const c = (category || "").toLowerCase();
  if (c.includes("extracurricular")) return "extracurricular";
  if (c.includes("physical education")) return "physicalEd";
  if (c.includes("transportation")) return "transportation";
  return null;
}
export function utCaps(award) {
  const a = Number(award) || 0;
  return [
    { key: "extracurricular", label: "Extracurricular", amount: Math.round(a * 0.20 * 100) / 100 },
    { key: "physicalEd", label: "Physical education", amount: Math.round(a * 0.20 * 100) / 100 },
    { key: "transportation", label: "Transportation", amount: 750 },
  ];
}
