...
// Only reasonably specific segments (multi-word or long) are safe to match as a
// bare substring of a whole message without producing false positives.
const SPECIFIC = SEGMENTS.map((segs) =>
  segs.filter((s) => s.length >= 6 && (s.includes(" ") || s.length >= 8))
);

// Scan a whole message for a specific vendor name/segment appearing verbatim,
// so "is Painting with a Twist direct pay?" is caught no matter the phrasing.
// Requires a multi-word / long segment to avoid matching generic words.
export function scanMessageForVendor(message) {
  const m = norm(message);
  if (m.length < 5) return null;
  let best = null;
  for (let i = 0; i < VENDORS.length; i++) {
    for (const seg of SPECIFIC[i]) {
      if (m.includes(seg) && (!best || seg.length > best.len)) {
        best = { v: VENDORS[i], len: seg.length };
      }
    }
  }
  return best ? { vendor: best.v, confident: true } : null;
}
