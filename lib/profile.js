...
export async function getProfile() {
  ...
  if (!profile) {
    // New accounts default to Arkansas (the program the app launched with); this
    // also lets the AR free-Family trigger flag them at creation.
    const { data } = await supabase
      .from("profiles")
      .insert({ user_id: user.id, is_parent: true, is_provider: false, state: "AR" })
      .select("*")
      .single();
    profile = data || { user_id: user.id, is_parent: true, is_provider: false, state: "AR" };
  }
  const { data: ent } = await supabase.from("entitlements").select("*").eq("user_id", user.id).single();
  const state = (profile?.state || "AR").toUpperCase();
  return { user, profile, plan: planFrom(ent, state), state, stateConfig: getStateConfig(state) };
}
