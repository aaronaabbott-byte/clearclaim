// Branded EFA invoice PDF. Matches the fields ADE's template asks for (vendor,
...
export async function buildInvoicePdf(inv, provider) {
  ...
  doc.text(p.business_name || p.provider_name || "Invoice", textX, ly); ly += 15;
  doc.setFont("helvetica", "normal"); doc.setFontSize(10.5); doc.setTextColor(...ink);
  const who = [p.provider_name, p.credentials].filter(Boolean).join(", ");
  if (who && p.business_name) { doc.text(who, textX, ly); ly += 13; }
  const contact = [p.contact_email, p.contact_phone, p.contact_website].filter(Boolean).join("   ·   ");
  if (contact) { doc.setFontSize(9.5); doc.setTextColor(...muted); doc.text(contact, textX, ly); ly += 12; }
  ...
  doc.text("Invoice for the Arkansas EFA program. Prepared with ClearClaim.", M, H - 28);
  return doc;
}
