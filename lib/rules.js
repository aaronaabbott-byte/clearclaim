...
export const PATHWAYS = [
  ...
  { value: "marketplace", label: "Marketplace — ordered through the ClassWallet marketplace" },
];
...
// The official ClassWallet reimbursement categories (purses). Pick the ONE that
// fits at submission. Musical instruments and furniture live under Educational
// Supplies now. Source: ClassWallet Categories sheet.
export const CATEGORIES = [
  "Curriculum",
  "Tuition and Fees",
  "Tutoring",
  "Instructional Services – Core or Co-Curricular",
  "Testing Costs",
  "Therapy Fees",
  "Educational Supplies",
  "Technology (computers, tablets, printers, and all tech-related accessories)",
  "Extra-Curricular, Physical Education, and Field Trips",
  "Transportation",
  "School Uniforms",
];
...
export function categoryCap(category, tier) {
  ...
  if (c.includes("travel") || c.includes("mileage") || c.includes("transportation"))
    return { key: "travel", label: "Travel / mileage", amount: caps.travel };
  return null;
}
// What a single claim contributes to its category cap. ADE counts the tech cap
// on the base price (pre-tax, pre-shipping), so prefer base_price when set;
// otherwise fall back to the full amount.
export function capContribution(c) {
  const base = c.base_price;
  const useBase = base !== null && base !== undefined && base !== "" && Number(base) > 0;
  return (useBase ? Number(base) : Number(c.amount)) || 0;
}
...
// Sum manually-logged cap entries (marketplace / already-approved spend that
// never became a claim) under the same cap in the same budget year. Entries use
// `entry_date`; fall back to `date` / created_at like claims do.
export function sumCapEntries(entries, kidId, capKey, by) {
  return (entries || [])
    .filter(e => e.kid_id === kidId && (categoryCap(e.category) || {}).key === capKey
      && inBudgetYear({ date: e.entry_date || e.date, created_at: e.created_at }, by))
...
const PG = [
  ...
  { kw: ["gardening hand tool", "garden hand tool", "garden trowel", "hand trowel"], max: null, eligible: false }, // banned — not on the approved list (corrected)
  // Furniture
  ...
  // Newly listed eligible items — updated ClassWallet Categories sheet. No price
  // guidance figure on the categories sheet, so no cap note; ok:1 confirms eligible.
  { kw: ["tonie box", "toniebox", "tonies", "tonie"], ok: 1 },
  { kw: ["yoto"], ok: 1 },
  { kw: ["3d printer filament", "3d filament", "printer filament", "filament"], ok: 1 },
  { kw: ["printer ink", "ink cartridge", "printer toner", "toner cartridge"], ok: 1 },
  { kw: ["printer paper"], ok: 1 },
  { kw: ["digital camera", "camera with accessories"], ok: 1 },
  { kw: ["culinary class", "culinary supplies"], ok: 1 },
  { kw: ["auto shop", "automotive shop"], ok: 1 },
  { kw: ["supplemental reading", "supplementary reading"], ok: 1 },
  { kw: ["art supplies", "art supply"], ok: 1 },
  { kw: ["sewing fabric", "sewing notions", "fabric and notions"], ok: 1 },
  // Added from the updated ClassWallet Categories sheet (rev. 2). Additive only.
  { kw: ["box kit", "science box kit", "activity box kit"], ok: 1 },
  // General PE equipment is eligible; competitive/athletic gear above stays ineligible.
  // Scoped to unambiguous PE terms so recreational "scooters"/"cones" aren't auto-marked eligible.
  { kw: ["hula hoop"], ok: 1 }, { kw: ["jump rope"], ok: 1 },
  { kw: ["playground ball"], ok: 1 }, { kw: ["agility cone"], ok: 1 },
  { kw: ["pe equipment", "physical education equipment"], ok: 1 },
];
// ---------------------------------------------------------------------------
// Split reimbursements. One-per-family capped items (a gym membership, a house-
// hold printer, a co-op fee, a garden bed) must be split across the funded
// students and each child's submission has to show the math. These helpers keep
// the arithmetic exact (work in cents) and produce the reviewer-facing note.
// ---------------------------------------------------------------------------
export function splitEqualCents(totalCents, n) {
  if (n <= 0) return [];
  const base = Math.floor(totalCents / n);
  const rem = totalCents - base * n;           // distribute the leftover pennies
  return Array.from({ length: n }, (_, i) => base + (i < rem ? 1 : 0));
}

// shares: [{ name, amount }] in dollars. Returns the reviewer breakdown string.
export function buildSplitNote(total, shares, method = "equal") {
  const money = (n) => `$${Number(n).toFixed(2)}`;
  const how = method === "equal"
    ? `Split equally across ${shares.length} funded students`
    : `Split across ${shares.length} funded students`;
  const lines = shares.map(s => `${s.name}: ${money(s.amount)}`).join("; ");
  return `${how}. Total ${money(total)} = ${lines}. This student's portion is claimed here; the same receipt and proof of payment are attached to each student's submission.`;
}
...
export function checkClaim(c) {
  ...
  if (pw === "marketplace") {
    push("ok","Marketplace order — ClassWallet now asks for order details on marketplace purchases. Keep the order confirmation and an itemized description. No out-of-pocket receipt or proof of payment needed.");
    c.vendor ? push("ok","Marketplace vendor entered.") : push("warn","Add the marketplace vendor (e.g. Best Buy).");
    c.items ? push("ok","Item description entered.") : push("warn","Describe the item(s) ordered.");
  } else if (pw === "directpay") {
  ...
  for (const g of PG) {
    ...
    if (g.eligible === false) {
      ...
      push("ok", `"${label}" is on the ClassWallet eligible-items list — keep an itemized receipt and a short note on how the student uses it.`);
    }
  ...
  if (cat.includes("technology")) {
    // The tech cap is counted on the base price (pre-tax, pre-shipping).
    const capAmt = capContribution(c);
    push(capAmt > 1000 ? "warn" : "ok", capAmt > 1000
      ? "Over the $1,000/student/year technology cap (all tech shares it: computers, tablets, printers, headphones, accessories). The cap counts the base price before tax and shipping. Above it needs a documented exception approved in advance."
      : "Counts toward the $1,000/student/year technology cap (base price, before tax and shipping). Note an e-reader counts as a tablet — you can't claim both.");
  }
  ...
  if (cat.includes("travel") || cat.includes("mileage") || cat.includes("transportation")) {
    // Mileage draws from one of two different 25% caps depending on the destination:
    // a field trip counts against the EXTRACURRICULAR cap, everything else (approved
    // class, school, tutor, therapy, EC activity) counts against the TRANSPORTATION cap.
    if (/field\s*trip/.test(hay)) {
      push("warn", `Mileage to a field trip is reimbursed at $${FUNDING.mileageRate}/mile but draws from the 25% EXTRACURRICULAR cap (~$${CAP_25}/student), NOT the transportation cap. Attach a completed mileage log (dates, destinations, miles) plus proof of attendance.`);
    } else {
      push("warn", `Mileage to and from an approved class, school, tutor, therapy, or extracurricular activity is reimbursed at $${FUNDING.mileageRate}/mile and draws from the 25% transportation cap (~$${CAP_25}/student). (A field trip is the exception — that mileage comes out of the extracurricular cap instead.) Attach a completed mileage log with dates, destinations, and miles.`);
    }
  }
  ...
  if (cat.includes("furniture") || /\b(desk|chair)\b/.test(hay)) {
    push("ok", `Desks and chairs (under Educational Supplies): one desk (≤$${FUNDING.deskMax}) and one chair (≤$${FUNDING.chairMax}) per student. No gaming or storage furniture.`);
  }

  // Specific high-frequency items with exact ADE rules.
  if (/chicken\s*coop|chicken\s*enclosure|rabbit\s*hutch|quail\s*(?:cage|pen|coop|hutch)|animal\s*(?:enclosure|pen|hutch|coop|husbandry)/.test(hay)) {
    push("warn", "Animal-husbandry enclosures — a chicken coop, rabbit hutch, or quail cage — are approved up to 50 sq ft AND under $300, and only WITH a curriculum that supports it. It's ONE per family/home and you must CHOOSE ONE type: you can fund a coop OR a rabbit hutch OR a quail cage, not more than one. Split the cost across all funded students in the lessons. Buy the enclosure and curriculum via Direct Pay from Lavender Vibes, Sunny Sprouts Education, or Learning Among the Pines, or do reimbursement instead (a pre-approval is recommended). It's the enclosure that's funded (with curriculum) — 'just the hutch,' same as a coop.");
  }
  if (/raised\s*garden\s*bed|garden\s*bed/.test(hay)) {
    push("warn", "A raised garden bed is approved up to $100 WITH a curriculum that supports the need. Soil, seeds, and plants for the lessons are reimbursable up to a combined $100 max. You can buy a bed and curriculum via Direct Pay from Lavender Vibes, Sunny Sprouts Education, or Learning Among the Pines, or do reimbursement instead (a pre-approval is recommended).");
  }

  // Theater performances (including live theater) are now banned in Arkansas.
  // Targets attending performances / tickets — not a student's own drama class.
  const theaterPerf =
    (/(theater|theatre)/.test(hay) && /(performance|show|production|play|musical|ticket|matinee|broadway|admission|attend|see )/.test(hay))
    || /live\s+(theater|theatre)/.test(hay)
    || /\bmatinee\b/.test(hay) || /\bbroadway\b/.test(hay) || /\bplay tickets\b/.test(hay)
    || /tickets?\s+(to|for)\s+(a\s+|the\s+)?(play|musical)\b/.test(hay);
  if (theaterPerf && !/(class|lesson|camp|course|instruction|workshop|tuition|program)/.test(hay))
    push("fail", "Theater performances — including live theater — are no longer reimbursable in Arkansas. Tickets to and admission for plays, musicals, and productions don't qualify.");

  // 3D printer filament: up to $300/year with a curriculum, no questions asked;
  // over $300/year needs an ordinary-and-necessary justification.
  if (/\bfilament\b/.test(hay)) {
    push("ok", "3D printer filament is approved up to $300/year WITH a supporting course or curriculum — no extra questions at or under $300. If your filament for the year goes over $300, the office will want specifics on how the extra is ordinary and necessary. (It's a consumable supply, separate from the 3D printer's tech-cap cost.)");
  }

  // Cricut / cameras are educational supplies (CTE), NOT technology — they do not
  // count toward the $1,000 tech cap. Flag if someone filed them as technology.
  if (/\bcricut\b|vinyl cutter|silhouette (?:cameo|machine)/.test(hay)) {
    push(cat.includes("technology") ? "warn" : "ok", "A Cricut / vinyl cutter is an educational supply (CTE), NOT technology — it does NOT count toward the $1,000 tech cap. File it under Educational Supplies. ADE price guidance is about $200.");
  }
  if (/\bcamera\b/.test(hay) && !/microscope/.test(hay)) {
    push(cat.includes("technology") ? "warn" : "ok", "A camera is an educational supply (CTE), NOT technology — it does NOT count toward the $1,000 tech cap. ADE price guidance for a camera with accessories is about $700.");
  }
  ...
  // Anything bought from a streaming company is never reimbursable in Arkansas —
  // subscriptions AND one-time title purchases/rentals — no matter how educational.
  // ADE confirmed even a documentary required for a lesson, bought/rented through a
  // streaming company because that's the only way to see it, does not qualify.
  // Checked before the general subscription note so "it's educational" can't soften it.
  const physicalMedia = /(dvd|blu-?ray|disc|physical copy|boxed set|box set)/.test(hay);
  const streamPlatform = /(netflix|hulu|disney\s*\+|disney plus|amazon prime|prime video|amazon video|amazon instant video|hbo\s*max|paramount\s*\+|peacock|apple tv|itunes|google play movie|youtube movie|youtube tv|sling tv|vudu|fandango at home|video on demand|\bvod\b|streaming service|streaming subscription)/.test(hay);
  const streamTitleBuy = /(movie|film|documentary|docuseries|feature film|mini-?series)/.test(hay)
    && /(rent|rental|renting|buy|bought|purchase|download|stream|streaming|digital)/.test(hay)
    && /(digital|online|stream|download|on demand|\bvod\b|apple tv|itunes|amazon|prime|google play|vudu|rent)/.test(hay);
  if (!physicalMedia && (streamPlatform || streamTitleBuy))
    push("fail", "Streaming purchases aren't reimbursable in Arkansas — subscriptions and one-time buys or rentals of a title alike (Netflix, Amazon Prime / Prime Video, Hulu, Disney+, Apple TV/iTunes, and any other streaming company). ADE has confirmed this even for a documentary or film required for a lesson that can only be viewed by buying it. (A physical DVD or a title inside an approved curriculum is a separate question.)");
  else if (/subscription/.test(hay) && !cat.includes("technology"))
  ...
  // Monthly / auto-billing subscriptions: the vendor often sends a billing
  // AGREEMENT, not a receipt — it doesn't show the payment method or the actual
  // charge, so proof of payment is missing.
  if (/subscription|auto-?bill|recurring|\bmonthly\b|per month|\/mo\b|time\s?4\s?learning|time for learning/.test(hay))
    push("warn", "Monthly/subscription reimbursement: if the vendor only sends an agreement or contract that doesn't SHOW the actual payment (no payment method, no charge, no $0 balance), that isn't a valid receipt by itself. Include a redacted bank or card statement showing the DATE and the actual charge. (Time4Learning is a common example — their document is a billing agreement, not a receipt.)");
