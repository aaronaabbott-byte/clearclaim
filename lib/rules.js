...
export function checkClaim(c) {
  ...
  if (/chicken\s*coop|chicken\s*enclosure/.test(hay)) {
    push("warn", "A chicken coop is approved up to 50 sq ft AND under $300, and only WITH a curriculum that supports it. The limit is per FAMILY, not per child — split the cost across all funded students in the lessons. You can buy a coop and curriculum via Direct Pay from Lavender Vibes, Sunny Sprouts Education, or Learning Among the Pines, or do reimbursement instead (a pre-approval is recommended for reimbursing this item).");
  }
